...
def generate_reports(jobs: list) -> tuple:
    ...
    for 100% free Cloudflare Pages static web hosting with mobile responsiveness.
    """
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; text-transform: none !important; }}
        ...
        .app-header {{ background-color: var(--bg-surface); border-bottom: 1px solid var(--border-subtle); padding: 16px 0; }}
        ...
        .brand-logo {{ width: 40px; height: 40px; background-color: rgba(37, 99, 235, 0.12); border: 1px solid rgba(37, 99, 235, 0.3); color: var(--brand-blue); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }}
        .brand-text h2 {{ font-size: 19px; font-weight: 600; color: var(--text-primary); }}
        .brand-text p {{ font-size: 13px; color: var(--text-secondary); }}
        .btn-search {{ display: inline-flex; align-items: center; justify-content: center; gap: 8px; background-color: var(--brand-blue); color: #ffffff; border: none; font-family: var(--font-family); font-size: 14px; font-weight: 600; padding: 10px 18px; border-radius: var(--radius-md); cursor: pointer; white-space: nowrap; text-decoration: none; }}
        .btn-search:hover {{ background-color: var(--brand-blue-hover); }}
        .main-content {{ max-width: 1200px; width: 100%; margin: 0 auto; padding: 24px 20px 60px 20px; }}
        .toolbar-panel {{ background-color: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); padding: 12px 16px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px; margin-bottom: 20px; }}
        .toolbar-left, .toolbar-right {{ display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }}
        .tab-group {{ display: flex; background-color: var(--bg-main); padding: 4px; border-radius: var(--radius-md); border: 1px solid var(--border-subtle); max-width: 100%; overflow-x: auto; }}
        ...
        .empty-state-box {{ grid-column: 1 / -1; background-color: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); padding: 48px 24px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; }}
        ...
        .job-card {{ background-color: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); padding: 20px; display: flex; flex-direction: column; justify-content: space-between; }}
        .job-card:hover {{ background-color: var(--bg-card-hover); border-color: var(--border-highlight); }}
        .source-tag {{ display: inline-block; padding: 3px 8px; border-radius: 6px; font-size: 11px; font-weight: 600; background-color: rgba(37, 99, 235, 0.15); color: #60a5fa; margin-bottom: 10px; }}
        .job-title {{ font-size: 16px; font-weight: 600; line-height: 1.35; margin-bottom: 10px; }}
        ...
        .skills-list {{ display: flex; flex-wrap: wrap; gap: 6px; margin-top: 12px; }}
        ...
        .card-footer {{ padding-top: 14px; border-top: 1px solid var(--border-subtle); display: flex; justify-content: space-between; align-items: center; }}
